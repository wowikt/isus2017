...
namespace Isus2017Spa.Tests
{
    public abstract class Isus2017SpaTestBase : AbpIntegratedTestBase<Isus2017SpaTestModule>
    {
        ...
        protected void UsingDbContext(Action<Isus2017SpaDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<Isus2017SpaDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<Isus2017SpaDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<Isus2017SpaDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<Isus2017SpaDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Isus2017SpaDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<Isus2017SpaDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<Isus2017SpaDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<Isus2017SpaDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<Isus2017SpaDbContext, Task<T>> func)
        {
            T result;
